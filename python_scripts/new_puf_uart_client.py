...
# PUF configuration
PUF_RESPONSE_BITS = 128  # Number of PUF cells in the FPGA design
PUF_RESPONSE_MASK = (1 << PUF_RESPONSE_BITS) - 1  # 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF for 128 bits
...
def read_response(ser, timeout=1.0):
    ser.timeout = timeout
    # The FPGA UART interface sends a full 128-bit (16-byte) response
    data = ser.read(16)
    if len(data) != 16:
        raise TimeoutError("Timed out waiting for 16-byte response")
    return data
...
def main():
    ...
    try:
        if args.cmd == "req":
            ...
            print(f"PUF response: 0x{value:032X} ({value:0{PUF_RESPONSE_BITS}b})")
            ...
            # 5. Query PUF count times - each PUF_REQ gets exactly one 16-byte response
            ...
            for i in range(args.count):
                ...
                if args.show_intermediate:
                    print(f"PUF response {i+1}: 0x{value:032X} ({value:0{PUF_RESPONSE_BITS}b})")
